...
custom_proxy_group=🚀 节点选择`select`[]♻️ 自动选择`[]🔯 故障转移`[]🔮 负载均衡`[]🌍 中国`[]🌍 节能`[]DIRECT`.*
...
custom_proxy_group=🎯 全球直连`select`[]DIRECT`[]🚀 节点选择`[]♻️ 自动选择`[]🔯 故障转移`[]🔮 负载均衡
custom_proxy_group=🐟 节点代理`select`[]🚀 节点选择`[]DIRECT`[]♻️ 自动选择`[]🔯 故障转移`[]🔮 负载均衡
