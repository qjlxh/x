...
ruleset=自定义同步,https://raw.githubusercontent.com/qjlxh/xxxx/main/wg.list
...
custom_proxy_group=节点选择`select`[]自动选择`[]负载均衡`[]字节跳动`[]中国`[]节能`[]莱垠`[]自定义同步`[]DIRECT`.*
...
custom_proxy_group=自定义同步`url-test`(🇺🇸|美国|us)`http://www.gstatic.com/generate_204`180,,50
custom_proxy_group=全球直连`select`[]DIRECT`[]节点选择`[]负载均衡`[]字节跳动`[]自定义同步
custom_proxy_group=节点代理`select`[]节点选择`[]DIRECT`[]负载均衡`[]字节跳动`[]自定义同步
